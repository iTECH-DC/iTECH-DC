...
from alice_core.voice.engine import get_engine as get_voice_engine
from alice_core.intelligence.assistant import handle as assistant_handle
...
SECURITY_LOG = DATA / "security_events.json"

def log_security_event(text, intent, refused, speech, tool=None, provider=None):
    """Append an owner-auditable security event (voice/chat command + outcome)."""
    try:
        items = read_json_file(SECURITY_LOG, [])
    ...
    items.insert(0, {
        ...
        "tool": tool,
        "provider": provider,
        "speech": str(speech or "")[:400],
    })
    write_json_file(SECURITY_LOG, items[:200])
    return items[0]
...
class AliceHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/api/security/audit-log":
            return self.send_json({"ok":True,"items":read_json_file(SECURITY_LOG, [])[:100]})
        ...
        if self.path == "/api/voice/capabilities":
            ...
            self.send_json({"ok": True, "offline": True,
                            "recognition": st["stt"]["available"], "synthesis": st["tts"]["available"],
                            "controller": "alice-offline-voice-engine",
                            "stt_backend": st["stt"]["backend"], "tts_backend": st["tts"]["backend"]})
            return

        if self.path == "/api/voice/status":
            return self.send_json(get_voice_engine().status())
        if self.path == "/api/voice/config":
            return self.send_json({"ok": True, "config": get_voice_engine().config()})
    ...
    def do_POST(self):
        # ---------------- Alice OS 14.0 offline voice + assistant ----------------
        if self.path == "/api/voice/transcribe":
            ...
            if length <= 0 or length > 16_000_000:
                return self.send_json({"ok": False, "error": "No audio received or clip too large."}, 400)
            try:
                rate = int(self.headers.get("X-Sample-Rate", "16000") or 16000)
            except Exception:
                rate = 16000
            pcm = self.rfile.read(length)
            result = eng.transcribe_pcm(pcm, sample_rate=rate)
            return self.send_json(result)
        if self.path == "/api/voice/speak":
            data = self.read_json()
            return self.send_json(get_voice_engine().speak(data.get("text", "")))
        if self.path == "/api/voice/tts":
            data = self.read_json()
            path = get_voice_engine().synthesize_wav(data.get("text", ""))
            if not path:
                return self.send_json({"ok": False, "error": "Text-to-speech engine unavailable."}, 503)
            try:
        ...
        if self.path == "/api/voice/stop":
            return self.send_json(get_voice_engine().stop_speaking())
        if self.path == "/api/voice/config":
            data = self.read_json()
            return self.send_json({"ok": True, "config": get_voice_engine().save_config(data)})
        if self.path == "/api/assistant/query":
            data = self.read_json()
            text = str(data.get("text", "")).strip()
            result = assistant_handle(text, context={"local_system": system_snapshot()},
                                      network_enabled=network_state.get("enabled", False),
                                      audit={"checks": []})
            try:
                log_intent(text, result.get("intent", "assistant"), result.get("speech", ""))
            except Exception:
                pass
            try:
                log_security_event(text, result.get("intent", "assistant"),
                                   result.get("intent") == "refused",
                                   result.get("speech", ""),
                                   tool=(result.get("data", {}) or {}).get("tool") if isinstance(result.get("data"), dict) else None,
